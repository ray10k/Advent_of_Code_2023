...
fn main() {
    ...
    if args.len() < 2 {
        panic!("Please provide the day(s) to run as a command-line argument, or the string \"all\" to run all days.");
    }
    ...
    if args[1] == "all" {
        ...
        let temp_days: Vec<u8> = args[1..].iter()
        .map(|x| x.parse().unwrap_or_else(|v| panic!("Not a valid day: {}", v)))
        .collect();
        days.extend(temp_days.iter());
    }
    let mut runtime = 0.0;
